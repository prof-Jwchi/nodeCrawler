...
class KopoAdmissionCrawler {
  ...
  async crawlAdmissionData() {
    try {
      ...
      console.log('Step 1: 모집과정 선택 (34 - 일학습병행)...');
      ...
      console.log('Step 2: 전체 캠퍼스 체크박스 클릭 (초기화)...');
      const step2Result = await this.page.evaluate(() => {
        ...
        if (checkbox) {
          ...
          return { success: true, checked: checkbox.checked, wasChecked: wasChecked };
        }
        return { success: false };
      });
      ...
      console.log('Step 4: 특정 대학 선택 확인 (1280000)...');
      const step4Result = await this.page.evaluate(() => {
        ...
        if (checkbox) {
          ...
          if (!checkbox.checked) {
            ...
            return { success: true, checked: checkbox.checked, wasChecked: wasChecked, action: 'checked' };
          } else {
            // 이미 체크되어 있으면 그대로 유지
            return { success: true, checked: checkbox.checked, wasChecked: wasChecked, action: 'already_checked' };
          }
        }
        return { success: false };
      });
